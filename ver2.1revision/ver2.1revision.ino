...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (dataFile) {
      ...
      logMessage += "Speed: " + String(speed) + " RPM";
      dataFile.println(logMessage);
      dataFile.close(); // Close the file
      Serial.println("Data logged: " + logMessage);

      // Send data to Google Sheets
      sendDataToGoogleSheets(temperature, humidity, accidentDetected ? "Accident Detected" : "No Accident", switchState == HIGH ? "NO COLLISION" : "Collision ALERT", speed);
